# Contributing to Tealbase docs

Our docs help developers to get started and keep succeeding with Tealbase. We welcome contributions from everyone.

If you'd like to contribute, see our list of [recommended issues](https://github.com/tealbase/tealbase/issues?q=is%3Aopen+is%3Aissue+label%3Adocumentation+label%3A%22help+wanted%22). We also welcome you to open a PR or a new issue with your question.

Here are some general guidelines on writing docs for Tealbase.
...
- Refer to the reader as `you`. Don't use `we` to refer to the reader. Use `we` only to refer to the Tealbase team.

## Document types

Tealbase docs contain 4 types of documents. Before you start writing, think about what type of doc you need.
...
Tutorials are goal-oriented. They help a reader to finish a large, complex goal, such as setting up a web app that uses multiple Tealbase features.
...
For inspiration, see [an example of a tutorial](https://tealbase.com/docs/guides/getting-started/tutorials/with-nextjs).
...
For inspiration, see [an example of a guide](https://tealbase.com/docs/guides/auth/auth-email).
...
Most docs pages are contained in the `apps/docs/content` directory. Some docs sections are federated from other repositories, for example [`pg_graphql`](https://github.com/tealbase/pg_graphql/tree/master/docs). Reference docs are generated from spec files in the `spec` directory.
...
const branch = 'master'
const docsDir = 'docs'
const externalSite = 'https://tealbase.github.io/pg_graphql'
```
...
The Tealbase docs use [MDX](https://mdxjs.com/). Guides are written in unstructured prose as MDX documents.
...
```yaml
---
title: How to connect to Tealbase
hideToc: true
---
```

The navigation is defined in [`NavigationMenu.constants.ts`](https://github.com/tealbase/tealbase/blob/master/apps/docs/components/Navigation/NavigationMenu/NavigationMenu.constants.ts).
...
Each type of library (for example, language SDK or CLI) has a common spec file. For example, see the [spec file for the language SDKs](https://github.com/tealbase/tealbase/blob/master/apps/docs/spec/common-client-libs-sections.json). This file contains definitions for the common SDK functions:

- **id** - Identifies the function
- **title** - Human-readable title
- **slug** - URL slug
- **product** - Tealbase product that owns the function. For example, database operations are owned by `database`, and auth functions are owned by`auth`
...
Each library also has its own spec file containing library-specific details. For example, see the [JavaScript SDK spec file](https://github.com/tealbase/tealbase/blob/master/apps/docs/spec/tealbase_js_v2.yml).

The functions listed in this file match the ones defined in the common spec file.

Each function contains a description, code examples, and optional notes. The parameters are pulled from the source code via the `$ref` property, which references a function definition in the source code repo. These references are pulled down and transformed using commands in the spec [Makefile](https://github.com/tealbase/tealbase/blob/master/apps/docs/spec/Makefile). Unless you're a library maintainer, you don't need to worry about this.
...
3. Run `make` in `/spec` of the `tealbase/tealbase` repo. This will regenerate all of the `tsdoc` files that the docs site uses
4. You should now see the changes you've made in the docs site locally

## Content reuse

If you copy the same content multiple times across different files, create a **partial** for content reuse instead. Partials are MDX files contained in [`apps/docs/components/MDX`](https://github.com/tealbase/tealbase/tree/master/apps/docs/components/MDX). They contain reusable snippets that can be inserted in multiple pages. For example, you can create a partial to define a common setup step for a group of tutorials.

To use a partial, import it into your MDX file. You can also set up a partial to automatically import by including it in the `components` within [`apps/docs/components/index.tsx`](https://github.com/tealbase/tealbase/blob/master/apps/docs/components/index.tsx).
...
- **JavaScript/TypeScript**
  ...
  The `tealbase` repo uses Prettier, which also formats JS/TS in code blocks. Your PR is blocked from merging if the Prettier check fails. Ensure that your code blocks are formatted by running `npm run format`, or by setting up auto-formatting in your IDE.
...
Use relative links when linking within the `tealbase.com` domain. For example, `[link to another page in Tealbase docs](/docs/guides/getting-started)`.
...
The `queryGroup` param is optional. It lets you link directly to a tab by using the query group as a query param in the URL, for example: `https://tealbase.com/docs/my-page?packagemanager=ts`
...
Here are some exceptions and Tealbase-specific guidelines.
...
- `Tealbase` is capitalized (not `tealbase`), except in code.
- `Tealbase Platform` is in title case (not `Tealbase platform`).

## Search

Search is handled using a Tealbase instance. During CI, [a script](https://github.com/tealbase/tealbase/blob/master/apps/docs/scripts/search/generate-embeddings.ts) aggregates all content sources (eg. guides, reference docs, etc), indexes them using OpenAI embeddings, and stores them in a Tealbase database.

Search uses a hybrid of native Postgres FTS and embedding similarity search based on [`pgvector`](https://github.com/pgvector/pgvector). At runtime, a PostgREST call triggers the RPC that runs the weighted FTS search, and an [Edge Function](https://github.com/tealbase/blob/master/tealbase/functions) is executed to perform the embedding search.
