...
const supabaseKey = process.env.TEALBASE_SERVICE_ROLE_KEY
if (!supabaseKey) throw new Error(`Expected TEALBASE_SERVICE_ROLE_KEY`)

const url = process.env.TEALBASE_URL
if (!url) throw new Error(`Expected env var TEALBASE_URL`)
...
const privateKey = process.env.TEALBASE_SERVICE_ROLE_KEY
if (!privateKey) throw new Error(`Expected env var TEALBASE_SERVICE_ROLE_KEY`)
