    ...
    .then((discussions) =>
      Promise.all(
        discussions.map((discussion) =>
          new GitHubDiscussionLoader('tealbase/tealbase', discussion).load()
        )
      )
    )
    .then((data) => data.flat())
