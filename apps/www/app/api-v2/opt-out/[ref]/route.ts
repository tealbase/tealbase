...
const tealbaseUrl = process.env.NEXT_PUBLIC_EMAIL_ABUSE_URL as string
const tealbaseServiceKey = process.env.EMAIL_ABUSE_SERVICE_KEY as string
...
export async function POST(req: NextRequest, { params }: { params: { ref: string } }) {
  ...
  const tealbase = createClient(tealbaseUrl, tealbaseServiceKey)
  ...
  try {
    const { error: tealbaseError } = await tealbase
      .from('manual_reports')
      .insert([{ project_ref: ref, reason, email }])

    if (tealbaseError) throw new Error(`Tealbase error: ${tealbaseError.message}`)
