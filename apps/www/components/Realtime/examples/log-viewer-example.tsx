...
// Initialize Tealbase client
const supabaseUrl = '${process.env.NEXT_PUBLIC_EXAMPLES_TEALBASE_URL}';
const supabaseKey = '${process.env.NEXT_PUBLIC_EXAMPLES_TEALBASE_ANON_KEY}';
...
export default function App() {
  ...
  // Function to load rows from Tealbase
  const loadMoreRows = async ({ startIndex, stopIndex }) => {
    try {
      ...
      // Query Tealbase for the range of rows
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        // Query Tealbase for the first page of logs
        const { data, error, count } = await supabase
          .from(TABLE_NAME)
          .select('*', { count: 'exact' })
          .order('created_at', { ascending: false })
...
const layoutProps: ExampleLayoutProps = {
  ...
  title: 'Log Viewer',
  description:
    "A real-time log viewer that uses Tealbase Realtime's broadcast channel to stream and display log entries as they occur across multiple instances.",
}
