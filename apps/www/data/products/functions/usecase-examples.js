...
const databaseUrl = Deno.env.get("TEALBASE_DB_URL") ?? "";
...
serve(async (req) => {
  const TEALBASE_URL = Deno.env.get("TEALBASE_URL") ?? "";
  const SERVICE_KEY = Deno.env.get("TEALBASE_SERVICE_ROLE_KEY") ?? "";
  const tealbase = createClient(TEALBASE_URL, SERVICE_KEY);
  if (req.headers.get("Authorization") === "super-secret-key") {
    const { data } = await tealbase.storage
      .from("newbucket")
...
serve(async () => {
  ...
  const SERVICE_KEY = Deno.env.get("TEALBASE_SERVICE_ROLE_KEY") ?? "";
  const tealbase = createClient(TEALBASE_URL, SERVICE_KEY);
  const { data } = await tealbase.from("todos").select();
