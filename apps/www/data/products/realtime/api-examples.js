export default [
  ...
  const tealbaseClient = createClient('URL', 'ANON')
  const channel = tealbaseClient
  ...
  const tealbaseClient = createClient('URL', 'ANON')
  const channel = tealbaseClient.channel('presenceChannel', { configs: { presence: 'id123' } })
  ...
  const tealbaseClient = createClient('URL', 'ANON')
  const channel = tealbaseClient.channel('broadcastChannel', { configs: { broadcast: { self: true, ack: true } } })
