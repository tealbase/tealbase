import { TealbaseClient } from '@supabase/supabase-js'
...
const supabaseUrl = process.env.NEXT_PUBLIC_TEALBASE_URL as string
const supabaseServiceKey = process.env.NEXT_PUBLIC_TEALBASE_ANON_KEY as string
...
export default async function handler(req: NextRequest) {
  ...
  if (!supabaseUrl) {
    return new Response(
      JSON.stringify({
        error:
          'No NEXT_PUBLIC_TEALBASE_URL set. Create this environment variable to use AI features.',
      }),
      {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
  ...
  if (!supabaseServiceKey) {
    return new Response(
      JSON.stringify({
        error:
          'No NEXT_PUBLIC_TEALBASE_ANON_KEY set. Create this environment variable to use AI features.',
      }),
      {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
...
async function handlePost(request: NextRequest) {
  ...
  const supabaseClient = new TealbaseClient(supabaseUrl, supabaseServiceKey)
