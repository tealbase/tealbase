...
const tealbase = createBrowserClient(
...
function App() {
  ...
  useEffect(() => {
    tealbase.auth.onAuthStateChange((event, session) => {
  ...
  return (
    <>
      <h1>Hono Tealbase Auth Example!</h1>
      <h2>Sign in</h2>
      {!user ? (
        <SignIn />
      ) : (
  ...
  return (
    <>
      <p>
        ...
        <a
          href="https://tealbase.com/docs/guides/auth/auth-anonymous"
          target="_blank"
        >
      ...
      <button
        type="button"
        onClick={async () => {
          const { data, error } = await tealbase.auth.signInAnonymously();
