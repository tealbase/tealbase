...
function App() {
  ...
  const invokeFunction = async () => {
    ...
    const { data, error } = await tealbase.functions.invoke(supaFunction, {
  ...
  return (
    <div className="p-2">
      <h2 className="mb-2 text-4xl">Tealbase Egde Functions Test Client</h2>
          ...
          {user ? (
            <div>
              ...
              <button
                ...
                onClick={() => tealbase.auth.signOut()}
              >
                Sign out
              </button>
            </div>
          ) : (
            <Auth tealbaseClient={tealbase} appearance={{ theme: ThemeSupa }} />
          )}
        </div>
      </div>
    </div>
