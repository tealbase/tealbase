...
import { TealbaseManagementAPI } from 'https://esm.sh/tealbase-management-js@0.1.2'

const config = {
  clientId: Deno.env.get('TEAL_CONNECT_CLIENT_ID')!,
  clientSecret: Deno.env.get('TEAL_CONNECT_CLIENT_SECRET')!,
  authorizationEndpointUri: 'https://api.tealbase.com/v1/oauth/authorize',
  tokenUri: 'https://api.tealbase.com/v1/oauth/token',
  redirectUri: 'http://localhost:54321/functions/v1/connect-tealbase/oauth2/callback',
}
...
router.get('/connect-tealbase', (ctx) => {
  ctx.response.body =
    'This is an example of implementing https://tealbase.com/docs/guides/integrations/oauth-apps/authorize-an-oauth-app . Navigate to /login to start the OAuth flow.'
})
router.get('/connect-tealbase/login', async (ctx) => {
...
router.get('/connect-tealbase/oauth2/callback', async (ctx) => {
  ...
  const supaManagementClient = new TealbaseManagementAPI({
