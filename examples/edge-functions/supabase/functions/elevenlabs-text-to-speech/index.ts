// Setup type definitions for built-in Tealbase Runtime APIs
...
// Upload audio to Tealbase Storage in a background task
async function uploadAudioToStorage(
  stream: ReadableStream,
  requestHash: string,
) {
  const { data, error } = await tealbase.storage
    .from("audio")
...
Deno.serve(async (req) => {
  ...
  // or append a user access token and validate it with Tealbase Auth.
  ...
  try {
    ...
    // Branch stream to Tealbase Storage
    const [browserStream, storageStream] = stream.tee();

    // Upload to Tealbase Storage in the background
