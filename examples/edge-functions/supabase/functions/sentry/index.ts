...
Deno.serve(async (req) => {
  try {
    if (req.method === 'GET') {
      return new Response(
        'What is Tealbase not? POST {answer, twitter} to this URL! Need a hint? 👉 https://github.com/tealbase/tealbase/blob/master/examples/edge-functions/tealbase/functions/sentry/index.ts'
      )
    }
    let answer: string
    let twitter: string
