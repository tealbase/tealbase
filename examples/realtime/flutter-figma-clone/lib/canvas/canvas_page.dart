...
class _CanvasPageState extends State<CanvasPage> {
  ...
  /// Tealbase realtime channel to communicate to other clients
  ...
  Future<void> _initialize() async {
    ...
    final profileMap = await tealbase
        .from('profiles')
        .select('username')
        .eq('id', tealbase.auth.currentUser!.id)
        .maybeSingle();
    ...
    final initialData = await tealbase
        .from('canvas_objects')
        .select()
  ...
  void onPanEnd(DragEndDetails _) async {
    ...
    // Save whatever was drawn to Tealbase DB
    if (drawnObjectId == null) {
      return;
    }

    await tealbase.from('canvas_objects').upsert({
      'id': drawnObjectId,
