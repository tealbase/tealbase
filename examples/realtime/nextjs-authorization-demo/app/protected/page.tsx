...
import { User, createClientComponentClient } from '@tealbase/auth-helpers-nextjs'
import { RealtimeChannel } from '@tealbase/tealbase-js'
...
export default function Chat() {
  ...
  const tealbase = createClientComponentClient()

  const getChannels = async () => {
    const channels = await tealbase.from('rooms').select('topic')
    setRooms(channels.data?.map(({ topic }) => topic) || [])
  }

  const addUserToChannel = async (email: string) => {
    const user = await tealbase.from('profiles').select('id').eq('email', email)
    if (!user.data?.length) {
      addMessage(true, true, `User ${email} not found`)
    } else {
      const room = await tealbase.from('rooms').select('topic').eq('topic', selectedRoom)
  ...
  useEffect(() => {
    tealbase.auth
      ...
      .then(async () => {
        ...
        const token = (await tealbase.auth.getSession()).data.session?.access_token!
        tealbase.realtime.setAuth(token)
        let main = tealbase
          .channel('supaslack')
  ...
  useEffect(() => {
    ...
    if (selectedRoom) {
      ...
      let newChannel = tealbase.channel(selectedRoom, {
        config: {
          broadcast: { self: true },
