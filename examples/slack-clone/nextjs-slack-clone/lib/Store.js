...
export const useStore = (props) => {
  ...
  useEffect(() => {
    ...
    const messageListener = tealbase
    ...
    const channelListener = tealbase
    ...
    // Cleanup on unmount
    return () => {
      tealbase.removeChannel(tealbase.channel(messageListener))
      tealbase.removeChannel(tealbase.channel(userListener))
      tealbase.removeChannel(tealbase.channel(channelListener))
    }
  }, [])
...
export const fetchChannels = async (setState) => {
  try {
    let { data } = await tealbase.from('channels').select('*')
...
export const fetchUser = async (userId, setState) => {
  try {
    let { data } = await tealbase.from('users').select(`*`).eq('id', userId)
    let user = data[0]
...
export const deleteChannel = async (channel_id) => {
  try {
    let { data } = await tealbase.from('channels').delete().match({ id: channel_id })
    return data
  } catch (error) {
    console.log('error', error)
  }
...
export const deleteMessage = async (message_id) => {
  try {
    let { data } = await tealbase.from('messages').delete().match({ id: message_id })
    return data
  } catch (error) {
    console.log('error', error)
  }
