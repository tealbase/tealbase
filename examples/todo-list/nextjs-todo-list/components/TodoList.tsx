...
import { Session, useTealbaseClient } from '@tealbase/auth-helpers-react'
import { useEffect, useState } from 'react'

type Todos = Database['public']['Tables']['todos']['Row']

export default function TodoList({ session }: { session: Session }) {
  const tealbase = useTealbaseClient<Database>()
  ...
  useEffect(() => {
    const fetchTodos = async () => {
      const { data: todos, error } = await tealbase
        .from('todos')
        .select('*')
  ...
  const addTodo = async (taskText: string) => {
    ...
    if (task.length) {
      const { data: todo, error } = await tealbase
        .from('todos')
  ...
  const deleteTodo = async (id: number) => {
    try {
      await tealbase.from('todos').delete().eq('id', id).throwOnError()
