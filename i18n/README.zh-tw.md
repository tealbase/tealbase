...
[Tealbase](https://tealbase.com)是一個開源的 Firebase 替代品。我們正在使用企業級的開源工具構建 Firebase 的功能。

- [x] 托管的 Postgres 資料庫。[文檔](https://tealbase.com/docs/guides/database)
- [x] 認證和授權。[文檔](https://tealbase.com/docs/guides/auth)
- [x] 自動生成的 API。
  - [x] REST.[文檔](https://tealbase.com/docs/guides/api#rest-api-overview)
  - [x] GraphQL。[文件](https://tealbase.com/docs/guides/api#graphql-api-overview)
  - [x] 實時訂閱。[文檔](https://tealbase.com/docs/guides/api#realtime-api-overview)
- [x] 函數。
  - [x] 資料庫函數。[文件](https://tealbase.com/docs/guides/database/functions)
  - [x] 邊缘功能 [文檔](https://tealbase.com/docs/guides/functions)
- [x] 文件存儲。[文件](https://tealbase.com/docs/guides/storage)
- [x] 儀表板
...
有關完整的文檔，請訪問[tealbase.com/docs](https://tealbase.com/docs)

要了解如何貢獻，請訪問[入門](../DEVELOPERS.md)

## 社群與支持

- [社群論壇](https://github.com/tealbase/tealbase/discussions)。最適合：幫助構建，討論資料庫的數佳實踐。
- [GitHub 問题](https://github.com/tealbase/tealbase/issues)。最適合：你在使用 Tealbase 时遇到的 bug 和错误。
- [電子郵件支持](https://tealbase.com/docs/support#business-support)。最適合：你的資料庫或數據基礎設施的問題。
- [Discord](https://discord.tealbase.com)。最適合：分享你的應用程式並與社群一起玩耍。

## 狀態

- [x] Alpha：我們正在與一组封閉的客户測試 Tealbase。
- [x] 公開 Alpha：任何人都可以在[tealbase.com/dashboard](https://tealbase.com/dashboard)上註冊。但請對我們寬容一些，有一些小問題。
- [x] 公開測試版：足夠穩定，適合大多數非企業使用的情况。
- [ ] 公開：普遍可用 [狀態](https://tealbase.com/docs/guides/getting-started/features#feature-status)
...
Tealbase 是一個開源工具的组合。我們正在使用企業級的開源產品來構建 Firebase 的功能。如果這些工具和社群存在，並且有 MIT、Apache 2 或同等的開放許可，我們將使用並支持該工具。如果該工具不存在，我們就自己建立並開放原始碼。Tealbase 不是 Firebase 的 1 對 1 映射。我們的目標是使用開源工具為開發者提供類似 Firebase 的開發者體驗。

**架構**

Tealbase 是一個[托管平台](https://tealbase.com/dashboard)。你可以註冊並開始使用 Tealbase，無需安裝任何東西。
你也可以[自行托管](https://tealbase.com/docs/guides/hosting/overview)和[本地開發](https://tealbase.com/docs/guides/local-development)。

![架構](https://github.com/tealbase/tealbase/blob/master/apps/docs/public/img/tealbase-architecture.svg)

- [PostgreSQL](https://www.postgresql.org/)是一個物件關係型資料庫系統，經過 30 多年的積極開發，它在可靠性、功能穩健性和性能方面赢得了良好的聲譽。
- [Realtime](https://github.com/tealbase/realtime)是一個 Elixir 服務器，允許你使用 websockets 監聽 PostgreSQL 的插入、更新和刪除。Realtime 對 Postgres 内置的複製功能進行投票，以了解資料庫的數位化，將變化轉换為 JSON，然后通過 websockets 將 JSON 廣播邊授權客户。
- [PostgREST](http://postgrest.org/)是一個網路服務器，它把你的 PostgreSQL 資料庫直接變成一個 RESTful API。
- [pg_graphql](http://github.com/tealbase/pg_graphql/)是一個 PostgreSQL 邊擴展，暴露了一個 GraphQL API。
- [Storage](https://github.com/tealbase/storage-api) 提供了一個 RESTful 接口來管理存儲在 S3 中的文件，使用 Postgres 來管理權限。
- [postgres-meta](https://github.com/tealbase/postgres-meta) 是一個用於管理你的 Postgres 的 RESTful API，允許你獲取表、添加角色和運行查詢等。
...
<table style="table-layout:fixed; white-space: nowrap;">
  <tr>
    <th>語言</th>
    <th>客户端</th>
    <th colspan="5">特徵-客户端(捆绑在Tealbase客户端中)</th>
  </tr>
...
- [英語](https://github.com/tealbase/tealbase)
...
[![新贊助商](https://user-images.githubusercontent.com/10214025/90518111-e74bbb00-e198-11ea-8f88-c9e3c1aa4b5b.png)](https://github.com/sponsors/tealbase)
