...
const TEALBASE_URL = process.env.NEXT_PUBLIC_TEALBASE_URL
const TEALBASE_ANON_KEY = process.env.NEXT_PUBLIC_TEALBASE_ANON_KEY
...
const useDocsSearch = () => {
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    fetch(`${TEALBASE_URL}/rest/v1/rpc/docs_search_fts`, {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
        ...(TEALBASE_ANON_KEY && {
          apikey: TEALBASE_ANON_KEY,
          authorization: `Bearer ${TEALBASE_ANON_KEY}`,
        }),
      },
    ...
    fetch(`${TEALBASE_URL}${FUNCTIONS_URL}search-embeddings`, {
      method: 'POST',
